...
Blinky::Blinky(int x, int y, QLabel *character, QGraphicsItem *parent) : Character(x, y, character, parent)
{
    //Start countdown for first created
    start = 81;

    animate = new QMovie(":/img/blinky_left.gif");
...
void Blinky::pic_change(){

    //Stop the QMovie
    animate->stop();

    //Default picture
    animate -> setFileName(":/img/blinky_left.gif");
    ...
    if(direction == X_NEGATIVE)
        ...
        animate -> setFileName(":/img/blinky_right.gif");
    //frightened mode
    if(scared > 0)
...
bool Blinky::move(Character* pacman){
...
bool Blinky::direction_controlled(QChar _key, Character* pacman, Character* ghost){
    ...
    //For binky, it chase the pacman directly.
    int x_distance, y_distance;

    if(pacman->x > x) x_distance = pacman->x - x;
    else x_distance = x - pacman->x;
    if(pacman->y > y) y_distance = pacman->y - y;
    else y_distance = y - pacman->y;

    if(y_distance == 0){
        if(x > pacman -> x) order[0] = X_NEGATIVE;
        else order[0] = X_POSITIVE;
        if(y > pacman -> y) order[1] = Y_NEGATIVE;
        ...
        if(y > pacman -> y) order[0] = Y_NEGATIVE;
        else order[0] = Y_POSITIVE;
        if(x > pacman -> x) order[1] = X_NEGATIVE;
